...
struct jpeg_compress_default:jpeg_compress_interface{
	const image_matrix_param& matrix;
	uint32_t row_stride;
	// 当前处理的源图像像素行数
	JDIMENSION next_line;
	jpeg_compress_default(const image_matrix_param& matrix):matrix(matrix),next_line(0){
...
void load_jpeg_mem(const std::vector<uint8_t> &jpeg_data,
		 jpeg_decompress_interface &decompress_instance);
image_matrix_param load_jpeg_mem(const uint8_t *jpeg_data,size_t size,
		const jpeg_custom_output_fun &custom=jpeg_custom_output_default);
image_matrix_param load_jpeg_mem(const std::vector<uint8_t> &jpeg_data,
		const jpeg_custom_output_fun &custom=jpeg_custom_output_default);
